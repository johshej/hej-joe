    ...
    @elseif (in_array($game->status->value, [GameStatus::Active->value, GameStatus::Scoring->value]))
        ...
        <div class="p-2 sm:p-4" style="{{ $cwStyle }}">
            ...
            <div class="mt-4 flex items-center justify-center gap-3">
                ...
                <button
                    x-data
                    @click="confirm('{{ __('Leave the game? You can return any time.') }}') && (window.location.href = '{{ route('games.index', ['current_team' => $teamModel->slug]) }}')"
                    class="flex items-center gap-1.5 rounded-lg px-2 py-1 text-sm text-zinc-400 transition hover:text-zinc-700 dark:hover:text-zinc-200"
                    type="button"
                >
